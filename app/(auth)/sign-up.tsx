...
const SignUp = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (error) {
      ...
      const emailError = errorItems.find(
        (item) => item.meta?.paramName === "emailAddress",
      );

      showAlert(
        passwordError ? "Password tidak aman" : "Tidak bisa membuat akun",
        passwordError?.longMessage ||
          emailError?.longMessage ||
          errorItems[0]?.longMessage ||
          "Coba periksa kembali detail yang Anda masukkan lalu ulangi lagi.",
      );
        ...
        navigate: ({ session, decorateUrl }) => {
          if (session?.currentTask) {
            showAlert(
              ...
              "Akun Anda berhasil dibuat, tetapi masih ada langkah tambahan yang perlu diselesaikan.",
            );
            return;
          }
  ...
  const handleResendCode = async () => {
    const resend = await signUp.verifications.sendEmailCode();
